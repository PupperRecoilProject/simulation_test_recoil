...
class Simulation:
    ...
    def __init__(self, config: AppConfig):
        ...
        try:
            ...
            # 在字串層面替換錯誤的路徑設定。
            # 這是一種非常可靠的方式，可以在不修改原始檔案的情況下解決路徑問題。
            # 注意：這裡我們假設您專案的檔案結構是 `assets/mesh/`
            ...
            # =========================================================================
            # === 【核心修改 2】在 Python 中強制覆寫控制器的類型                  ===
            # =========================================================================
            # 遍歷所有致動器 (actuators)
            for i in range(self.model.nu):
                # 將每個致動器的 biastype 強制設定為 'affine' (2)
                # 'affine' 允許我們使用 biasprm 來實現 PD 控制
                # 就算 XML 中設定為 'none' (0)，也會被這裡的程式碼覆寫
