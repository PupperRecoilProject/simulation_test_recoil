...
## 一句話現況 / 目標
- **目標**：重啟做出一個「完整可用、設計漂亮、功能人性化」的四足機器狗系統；積極重構，不接續舊目標。
- **現況**（2026-06-04）：已完成跨 repo 介面契約釐清（roll/pitch 等）、文件與工作流程地基；
  下一階段＝重構（採 headless 核心+薄客戶端架構）、補測試、清理將就設計。尚未動控制路徑程式碼。
- **最高 CP 待辦**：修 `hardware_controller._execute_stop` self-join bug（=退出硬體卡死，可自主修+測）。
