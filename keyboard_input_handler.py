...
class KeyboardInputHandler:
    ...
    def key_callback(self, window, key, scancode, action, mods):
        ...
        # Mode-specific Actions (can be PRESS or REPEAT)
        ...
        if self.state.control_mode == "MANUAL_CTRL":
            if key == glfw.KEY_1: self.state.manual_ctrl_index = (self.state.manual_ctrl_index - 1) % 12
            elif key == glfw.KEY_2: self.state.manual_ctrl_index = (self.state.manual_ctrl_index + 1) % 12
        ...
        # Parameter Tuning and Movement (only in KEYBOARD input mode and applicable control modes)
        if self.state.input_mode != "KEYBOARD":
            return
            
        if self.state.control_mode in ["WALKING", "FLOATING"]:
