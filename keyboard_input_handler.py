...
class KeyboardInputHandler:
    ...
    def key_callback(self, window, key, scancode, action, mods):
        # Universal Actions (triggered on PRESS only)
        ...
        if self.state.control_mode == "JOINT_TEST":
            if key == glfw.KEY_LEFT_BRACKET: self.state.joint_test_index = (self.state.joint_test_index - 1) % 12
            elif key == glfw.KEY_RIGHT_BRACKET: self.state.joint_test_index = (self.state.joint_test_index + 1) % 12
        ...
        if self.state.control_mode == "MANUAL_CTRL":
            if key == glfw.KEY_LEFT_BRACKET: self.state.manual_ctrl_index = (self.state.manual_ctrl_index - 1) % 12
            elif key == glfw.KEY_RIGHT_BRACKET: self.state.manual_ctrl_index = (self.state.manual_ctrl_index + 1) % 12
            ...
            elif key == glfw.KEY_C: self.state.manual_final_ctrl.fill(0.0)
            return

        if self.state.input_mode != "KEYBOARD" or self.state.control_mode not in ["WALKING", "FLOATING"]:
            return
            
        # Parameter Tuning and Movement
